...
# Hardcoded folders
folders = [
    "results/_run_logs/ml_fcs_vary_alpha",
...
fig.suptitle('Tradeoff in Subsidized Best Reward Setting', fontsize=18, fontweight='bold', y=1.02)
...
sorted_files_folder = sorted(pathlib.Path(folders[0]).iterdir(), key=lambda x: x.name)
nseeds = -1
...
sorted_files_folder = sorted(pathlib.Path(folders[1]).iterdir(), key=lambda x: x.name)
nseeds = -1
...
axs[1].legend(handles=legend_elements, loc='lower right', fontsize=14, ncol=2)
...
plt.savefig(save_dir + "/tradeoff_fcs.pdf", bbox_inches="tight")
plt.close()
