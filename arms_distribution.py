...
for i, t in enumerate(x_points):
    for algo_index, label in enumerate(selected_algos):
        ...
        if not cur_data.empty:
            ...
            nsamps_array[algo_index, i, :] = nsamps_values
...
for i, t in enumerate(x_points):
    ...
    for algo_index, label in enumerate(selected_algos):
        ...
        plt.bar(x_positions, nsamps_array[algo_index, i, :], width=bar_width, color=COLORS[algo_index], alpha=0.5, label=label)
    ...
    plt.legend()
    plt.xticks(np.arange(narms) + bar_width / 2, np.arange(narms))
    plt.yticks()
    ...
    frame_filename = os.path.join(tmp_frames_dir, f'frame_{t}.png')
