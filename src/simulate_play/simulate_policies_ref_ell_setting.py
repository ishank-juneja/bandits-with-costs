...
import sys
import argparse
from src.policy_library.policy_library import (improved_ucb, asymmetric_pe, pairwise_elimination,
                                               ref_arm_ell_UCB, UCB, pe_new)
    ...
    for al in algos:
        for rs in range(nruns):
            ...
            if al == 'ucb':
                ...
                for t in range(1, horizon + 1):
                    # Receive the arm index to sample, and the updated delta_tilde, and episode number
                    k, delta_tilde, episode = pe_new(ref_ell_idx=ref_arm_ell , mu_hat=mu_hat, nsamps=nsamps,
                                                     horizon=horizon, delta_tilde=delta_tilde, episode_num=episode,
                                                     last_sampled=last_sampled)
                    # Update the last sampled arm index
                    last_sampled = k
                    # Do book-keeping for this policy, and receive all the params that were modified
