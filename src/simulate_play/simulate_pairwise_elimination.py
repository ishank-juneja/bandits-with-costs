...
if __name__ == '__main__':
    ...
    ref_arm_ell = instance_data.get('arm_ell', None)
    if ref_arm_ell is None:
        raise ValueError("No arm_ell found in the input file")
    # Perform pruning by removing arms with cost higher than arm ell
    arm_reward_array = arm_reward_array[:ref_arm_ell + 1]
    arm_cost_array = arm_cost_array[:ref_arm_ell + 1]
    n_arms = len(arm_reward_array)
    # get the action against which we calibrate reward (here ref arm ell)
    k_calib = ref_arm_ell
    ...
    for al in algos:
        for rs in range(nruns):
            ...
            if al == 'ucb':
                ...
                # Begin policy loop
                for t in range(1, horizon + 1):
                    k, delta_tilde = pairwise_elimination(mu_hat, nsamps, horizon, delta_tilde, B, last_sampled)
                    # Update the last sampled arm index
                    last_sampled = k
                    # Do book-keeping for this policy, and receive all the params that were modified
                    nsamps, mu_hat, reg = do_bookkeeping_conventional(STEP=STEP, arm_samples=arm_samples, k=k, t=t, nsamps=nsamps,
